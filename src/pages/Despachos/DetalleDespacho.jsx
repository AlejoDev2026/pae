...
const TIPOS_ARCHIVO = [
    { key: "AM", label: "AM", color: "bg-emerald-50 border-emerald-200 text-emerald-700" },
    { key: "PM", label: "PM", color: "bg-sky-50 border-sky-200 text-sky-700" },
    { key: "JORNADA_UNICA", label: "Jornada Única", color: "bg-violet-50 border-violet-200 text-violet-700" },
];
...
export const DetalleDespacho = ({ setSidebar, navegar }) => {
    ...
    const obtenerDetalle = async () => {
        ...
        try {
            ...
            Object.values(archivos).forEach((listaArchivos) => {
    ...
    const resumenGeneral = useMemo(() => {
        ...
        Object.values(archivos).forEach((listaArchivos) => {
            const resumen = contarResumenTipo(listaArchivos || []);
                ...
                <table className="min-w-max w-full text-sm border-separate border-spacing-0">
                    ...
                    <tbody>
                        {filas.map((fila, index) => (
                            <tr key={`${fila.consecutivo}-${fila.producto}-${fila.unidadCobertura}`} className="hover:bg-slate-50">
    ...
    const renderArchivosPorTipo = (tipoInfo) => {
        const archivos = detalle?.archivos?.[tipoInfo.key] || [];
        const abierta = !!seccionesAbiertas[tipoInfo.key];
        ...
        return (
            <section className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
                <button
                    type="button"
                    onClick={() => toggleSeccion(tipoInfo.key)}
                    className={`w-full px-5 py-4 border-b flex items-center justify-between gap-4 ${tipoInfo.color}`}
                >
                    <div className="text-left">
                        <h2 className="text-lg font-bold">{tipoInfo.label}</h2>
                    ...
                    <div className="p-5">
                        {archivos.length === 0 ? (
                            <div className="text-slate-500 text-sm">
                                No hay archivos cargados para {tipoInfo.label}.
                            </div>
                        ) : (
                                        ...
                                        >
                                            ...
                                            >
                                                <div className="flex items-start gap-3 text-left">
                                                    <FaFileExcel className="text-emerald-600 mt-1" />
                ...
                {loading ? (
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar pr-1 flex flex-col gap-6">
                        <section className="border border-slate-200 rounded-2xl p-5 bg-slate-50">
                            <div className="w-full bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-5">
                            {TIPOS_ARCHIVO.map((tipoInfo) => renderArchivosPorTipo(tipoInfo))}
                        </div>
                    </div>
                )}
            </article>
