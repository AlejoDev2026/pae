...
import { ExcelInformeRutaEspecialConsolidadoExporter }
    from "../../informes/exporters/ExcelInformeRutaEspecialConsolidadoExporter";
...
export const InformeRutaEspecialConsolidadoViewRoldanillo = ({
    setSidebar,
    navegar,
    idDespacho,
    idCategoria = 21,
        ...
        const cargarInforme = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        response?.mensaje ||
                        "No fue posible cargar el informe consolidado por rutas especiales."
                    );
                }
                ...
                setError(
                    err?.message ||
                    "Ocurrió un error al consultar el informe consolidado por rutas especiales."
                );
            } finally {
                if (activo) {
    ...
    const resumenInformeActivo = useMemo(() => {
        return {
            titulo: "Informe consolidado por rutas especiales",
            descripcion:
    ...
    const exportarExcel = async () => {
        try {
            ...
            await ExcelInformeRutaEspecialConsolidadoExporter.exportar({
                data: dataExport,
            });
        ...
        if (loading) {
            return (
                <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-500">
                    Cargando informe consolidado por rutas especiales...
                </div>
            );
        }
...
export default InformeRutaEspecialConsolidadoView;