...
} from "../Inventarios/OrdenesAlistamiento/ordenesAlistamientoHelpers";
...
export const InformesDespacho = ({ setSidebar, navegar }) => {
    ...
    const obtenerTipoInicial = () => {
        const tipoGuardado = localStorage.getItem("tipoInformeInicial");
    ...
    const [generandoOrdenAlistamiento, setGenerandoOrdenAlistamiento] = useState(false);
    ...
    useEffect(() => {
        const obtenerInformeRutaFormato = async () => {
            ...
            try {
                ...
                const url = `${API_BASE}Despachos/DespachosGetInformeRutaFormato.php?idDespacho=${idDespacho}&tipo=POR_RUTA`;
                console.log('url', url);
    ...
    useEffect(() => {
        const obtenerInformeCadenaFrio = async () => {
            ...
            try {
                ...
                const url = `${API_BASE}Despachos/DespachosGetInformeRutaCategoriaColegio.php?idDespacho=${idDespacho}&idCategoria=21`;
                console.log('url', url);
    ...
    const generarOrdenAlistamiento = async () => {
        if (generandoOrdenAlistamiento) return;

        const idDespachoNumero = Number(idDespacho || 0);
        ...
        if (!idUsuario) {
            toast.error("No fue posible identificar el usuario de la sesión");
            return;
        }

        if (!(informePorJornada?.productos || []).length) {
            toast.info("No hay productos para generar la orden de alistamiento");
            return;
        }

        const confirmaGeneracion = window.confirm(
            "Se generar\u00e1 la orden de alistamiento y se reservar\u00e1 inventario para todos los productos del despacho. \u00bfDeseas continuar?"
        );

        if (!confirmaGeneracion) return;

        setGenerandoOrdenAlistamiento(true);

        try {
            const resultado = await solicitarOrdenesJson(
                ENDPOINTS_ORDENES_ALISTAMIENTO.generar,
                {
                    method: "POST",
                    body: {
                        idDespachoInforme: idDespachoNumero,
                        idDespacho: idDespachoNumero,
                        idUsuario,
                    },
                }
            );
            ...
            const ordenGenerada =
                resultado?.data?.orden || resultado?.data || null;
            ...
            if (guardarOrdenSeleccionada(ordenGenerada)) {
                estadoPagina("DetalleOrdenAlistamientoInventario");
            }
        } catch (error) {
            console.error("Error generando orden de alistamiento:", error);

            const ordenExistente = error?.data?.orden || error?.data;
            if (
                error?.status === 409 &&
                obtenerIdOrden(ordenExistente)
            ) {
                const verOrden = window.confirm(
                    `${error?.message || "Ya existe una orden de alistamiento para este despacho."}\n\n\u00bfDeseas ver la orden existente?`
                );

                if (
                    verOrden &&
                    guardarOrdenSeleccionada(ordenExistente)
                ) {
                    estadoPagina("DetalleOrdenAlistamientoInventario");
                }
                return;
            ...
            toast.error(
                error?.message ||
                "No se pudo generar la orden de alistamiento"
            );
        } finally {
            setGenerandoOrdenAlistamiento(false);
        }
    };
                                ...
                                <section className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm shrink-0">
                                    <div className="px-4 md:px-5 py-4 border-b bg-slate-50 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {informeActivo === "POR_JORNADA" && (
                                                <button
                                                    type="button"
                                                    onClick={generarOrdenAlistamiento}
                                                    disabled={
                                                        generandoOrdenAlistamiento ||
                                                        loadingJornadaContrato ||
                                                        !(informePorJornada?.productos || []).length
                                                    }
                                                    className="px-4 py-2 bg-blue-800 text-white rounded-lg hover:bg-blue-900 transition-colors flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                                                >
                                                    <FaClipboardCheck />
                                                    <span>
                                                        {generandoOrdenAlistamiento
...
export default InformesDespacho;
