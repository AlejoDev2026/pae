...
export const EditarUbicacionInventario = ({ setSidebar, navegar, data }) => {
    const abrirMenu = () => setSidebar(true);
    const estadoPagina = (pagina) => navegar(pagina);

    const dataLocal = (() => {
        try {
            return JSON.parse(localStorage.getItem("ubicacionInventarioEditar"));
        } catch {
            return null;
        }
    })();

    const ubicacionEditar = data || dataLocal;
    ...
    const [form, setForm] = useState({
        id: ubicacionEditar?.id || "",
        idBodega: ubicacionEditar?.idBodega || "",
        codigo: ubicacionEditar?.codigo || "",
        nombre: ubicacionEditar?.nombre || "",
        descripcion: ubicacionEditar?.descripcion || "",
        estado: Number(ubicacionEditar?.estado ?? 1),
    });
    ...
    const cargarBodegas = async () => {
        try {
            ...
            setBodegas(Array.isArray(json.data) ? json.data : []);
        } catch (error) {
            console.error(error);
    ...
    useEffect(() => {
        if (!ubicacionEditar?.id) {
            toast.warning("No se recibió la ubicación seleccionada");
    ...
    const validarFormulario = () => {
        if (!form.id) {
            toast.warning("No se encontró el ID de la ubicación");
            return false;
        }
    ...
    const guardarUbicacion = async (e) => {
        ...
        try {
            ...
            if (!response.rpta) {
                toast.error(response.mensaje || "No se pudo actualizar la ubicación");
                return;
            }

            localStorage.removeItem("ubicacionInventarioEditar");
            toast.success(response.mensaje || "Ubicación actualizada correctamente");
            estadoPagina("UbicacionesInventario");
        } catch (error) {
            console.error(error);
            toast.error("Ocurrió un error al actualizar la ubicación");
        } finally {
            setGuardando(false);
        }
    };
    ...
    return (
        <div className="w-full h-screen flex flex-col p-6 gap-6 bg-slate-100">
            <div className="flex items-center gap-5 lg:hidden">
                ...
                <h1 className="text-lg font-bold">Editar ubicación</h1>
            </div>
                                ...
                                <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                                    <div className="px-5 py-4 border-b border-slate-200 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">
                                                Modifica la bodega, código, nombre o estado operativo.
                                            </p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <span className="text-xs text-slate-400">
                                                    Bodega donde está asociada la ubicación.
                                                </span>
                                            </div>
                                            ...
                                            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                                                ...
                                                <p className="mt-1 text-sm font-semibold text-slate-800 break-words">
                                                    {bodegaSeleccionada
                                                        ...
                                                        : ubicacionEditar?.nombreBodega ||
                                                        ubicacionEditar?.bodegaNombre ||
                                                        ubicacionEditar?.bodega ||
                                    ...
                                    <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                                        ...
                                        <div className="p-5">
                                            ...
                                            <p className="mt-3 text-sm text-slate-500 leading-relaxed">
                                                Mantén nombres y códigos consistentes para que los
                                                movimientos, conteos y existencias se identifiquen con
                                                claridad.
                                            </p>
                                        </div>
                                    </div>
