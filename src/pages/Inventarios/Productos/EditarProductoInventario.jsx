...
export const EditarProductoInventario = ({ setSidebar, navegar, data }) => {
  const abrirMenu = () => setSidebar(true);
  const estadoPagina = (pagina) => navegar(pagina);

  const dataLocal = (() => {
    try {
      return JSON.parse(localStorage.getItem("productoInventarioEditar"));
    } catch {
      return null;
    }
  })();

  const productoEditar = data || dataLocal;
  ...
  const [form, setForm] = useState({
    ...
    descripcion:
      productoEditar?.descripcion || productoEditar?.nombreProducto || "",
    idGrupo: productoEditar?.idGrupo || "",
    idEmbalaje: productoEditar?.idEmbalaje || "",
    observacionProducto: productoEditar?.observacionProducto || "",
    estadoProducto: Number(productoEditar?.estadoProducto ?? 1),

    idTipoProductoInventario:
      productoEditar?.idTipoProductoInventario || "",
    manejaLote: Number(productoEditar?.manejaLote ?? 0),
    manejaVencimiento: Number(productoEditar?.manejaVencimiento ?? 0),
    stockMinimo: productoEditar?.stockMinimo ?? 0,
    stockMaximo:
      productoEditar?.stockMaximo === null ||
      productoEditar?.stockMaximo === undefined
        ? ""
        : productoEditar?.stockMaximo,
    unidadBaseInventario: productoEditar?.unidadBaseInventario || "UND",
    observacionInventario: productoEditar?.observacionInventario || "",
    estadoConfig: Number(
      productoEditar?.estadoConfig ?? productoEditar?.estadoProducto ?? 1
    ),
  });
  ...
  useEffect(() => {
    if (!productoEditar?.idProducto) {
      toast.warning("No se recibió el producto seleccionado");
  ...
  const validarFormulario = () => {
    if (!form.idProducto) {
      toast.warning("No se encontró el ID del producto");
      return false;
    }
  ...
  const guardarProducto = async (e) => {
    ...
    try {
      ...
      if (!json.rpta) {
        toast.error(json.mensaje || "No se pudo actualizar el producto");
        return;
      }

      localStorage.removeItem("productoInventarioEditar");
      toast.success(json.mensaje || "Producto actualizado correctamente");
      estadoPagina("ProductosInventario");
    } catch (error) {
      console.error(error);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Actualiza el producto base y su configuración de inventario.
                </p>
              </div>
            </div>
                ...
                <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-200 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Datos base guardados en el catálogo de productos.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <span className="text-xs text-slate-400">
                        Este nombre será visible en catálogo e inventario.
                      </span>
                    </div>
                ...
                <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="p-5 space-y-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <span className="text-xs text-slate-400">
                        Al cambiar el tipo, se actualizan las reglas base de
                        lote y vencimiento.
                      </span>
                    </div>
                      ...
                      <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-800">
                          {grupoSeleccionado?.codigo ||
                            productoEditar?.codigoGrupo ||
                            "Sin grupo"}
                        </p>

                        <p className="text-xs text-slate-500 mt-0.5">
                          {grupoSeleccionado?.categoriaProducto ||
                            productoEditar?.categoriaProducto ||
                            "Sin categoría"}
                        </p>
                      </div>
                      ...
                      <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-800 break-words">
                          {embalajeSeleccionado
                            ? `${embalajeSeleccionado.productoBase || ""} ${
                                embalajeSeleccionado.presentacion || ""
                              }`
                            : productoEditar?.embalajeProductoBase ||
                              "Sin embalaje"}
                        </p>

                        {(embalajeSeleccionado?.embalaje ||
                          productoEditar?.embalajeNombre) && (
                          <p className="text-xs text-slate-500 mt-0.5">
                            {embalajeSeleccionado?.embalaje ||
                              productoEditar?.embalajeNombre}
                            {embalajeSeleccionado?.uniCaja ||
                            productoEditar?.uniCaja
                              ? ` / ${
                                  embalajeSeleccionado?.uniCaja ||
                                  productoEditar?.uniCaja
                                } und.`
                      ...
                      <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-800 break-words">
                          {tipoSeleccionado
                            ...
                            : productoEditar?.tipoProductoInventario ||
                  ...
                  <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="p-5">
                      ...
                      <p className="mt-3 text-sm text-slate-500 leading-relaxed">
                        Si cambias el tipo de producto, revisa nuevamente lote,
                        vencimiento y unidad base antes de guardar.
                      </p>
                    </div>
                  </div>
                </div>
