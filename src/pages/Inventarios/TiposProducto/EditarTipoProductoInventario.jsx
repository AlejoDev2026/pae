...
export const EditarTipoProductoInventario = ({ setSidebar, navegar, data }) => {
    const abrirMenu = () => setSidebar(true);
    const estadoPagina = (pagina) => navegar(pagina);

    const dataLocal = (() => {
        try {
            return JSON.parse(localStorage.getItem("tipoProductoInventarioEditar"));
        } catch {
            return null;
        }
    })();

    const tipoEditar = data || dataLocal;

    const [guardando, setGuardando] = useState(false);

    const [form, setForm] = useState({
        id: tipoEditar?.id || "",
        codigo: tipoEditar?.codigo || "",
        nombre: tipoEditar?.nombre || "",
        descripcion: tipoEditar?.descripcion || "",
        manejaLote: Number(tipoEditar?.manejaLote ?? 0),
        manejaVencimiento: Number(tipoEditar?.manejaVencimiento ?? 0),
        requiereFechaVencimiento: Number(tipoEditar?.requiereFechaVencimiento ?? 0),
        requiereBodegaFria: Number(tipoEditar?.requiereBodegaFria ?? 0),
        estado: Number(tipoEditar?.estado ?? 1),
    });
    ...
    useEffect(() => {
        if (!tipoEditar?.id) {
            toast.warning("No se recibió el tipo de producto seleccionado");
    ...
    const validarFormulario = () => {
        if (!form.id) {
            toast.warning("No se encontró el ID del tipo de producto");
            return false;
        }
    ...
    const guardarTipoProducto = async (e) => {
        ...
        try {
            ...
            if (!response.rpta) {
                toast.error(
                    response.mensaje || "No se pudo actualizar el tipo de producto"
                );
                return;
            }

            localStorage.removeItem("tipoProductoInventarioEditar");
            toast.success(
                response.mensaje || "Tipo de producto actualizado correctamente"
            );
            estadoPagina("TiposProductoInventario");
        } catch (error) {
            console.error(error);
            toast.error("Ocurrió un error al actualizar el tipo de producto");
        } finally {
            setGuardando(false);
        }
    };
    ...
    return (
        <div className="w-full h-screen flex flex-col p-6 gap-6 bg-slate-100">
            <div className="flex items-center gap-5 lg:hidden">
                ...
                <h1 className="text-lg font-bold">Editar tipo de producto</h1>
            </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">
                                    Actualiza las reglas base del tipo de producto seleccionado.
                                </p>
                            </div>
                        </div>
                                ...
                                <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                                    <div className="px-5 py-4 border-b border-slate-200 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">
                                                Modifica identificación, control de lote, vencimiento y
                                                bodega fría.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                                        ...
                                        <div className="p-5">
                                            ...
                                            <p className="mt-3 text-sm text-slate-500 leading-relaxed">
                                                Recuerda que este tipo será usado posteriormente para
                                                configurar productos del catálogo en el módulo de
                                                inventario.
                                            </p>
                                        </div>
                                    </div>
