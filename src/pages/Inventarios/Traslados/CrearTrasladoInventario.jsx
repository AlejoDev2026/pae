...
export const CrearTrasladoInventario = ({ setSidebar, navegar }) => {
    ...
    const obtenerTrasladoLocal = (esVista = false) => {
        const llaves = esVista
            ? ["trasladoInventarioDetalle", "trasladoInventarioEditar"]
            : ["trasladoInventarioEditar", "trasladoInventarioDetalle"];
        ...
        for (const llave of llaves) {
            ...
            try {
                const traslado = JSON.parse(valor);
                const id = Number(
    ...
    useEffect(() => {
        const modo = String(
            localStorage.getItem("trasladoInventarioModo") || ""
        ).toUpperCase();
        ...
        const trasladoLocal = obtenerTrasladoLocal(esVista);
    ...
    const soloLectura =
        modoDetalle ||
        (modoEdicion && estadoDocumento !== "BORRADOR");
    ...
    const cargarTrasladoEditar = useCallback(async () => {
        ...
        try {
            ...
            setEstadoDocumento(estadoProceso);
            ...
            if (
                ...
                setModoDetalle(true);
                toast.info(
                    "Este traslado ya fue finalizado. Se abrirá en modo solo lectura."
                );
            }
    ...
    }, [idDocumentoEdicion, estadoPagina, modoDetalle]);
    ...
    const buscarProducto = async () => {
        if (soloLectura) {
            toast.info("Este traslado está en modo solo lectura.");
            return;
        }
    ...
    const volver = () => {
        localStorage.removeItem("trasladoInventarioEditar");
        localStorage.removeItem("trasladoInventarioModo");
        estadoPagina("TrasladosInventario");
    };
                                        ...
                                        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                                            ...
                                            <div className="p-4 space-y-3">
                                                ...
                                                {soloLectura && (
                                                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                                        <div className="flex items-start gap-2">
                                                            <FaCheckCircle className="mt-0.5 text-slate-600 shrink-0" />
                                                            <p className="text-xs text-slate-700 leading-5">
                                                                Este traslado se está visualizando en modo detalle. Los campos están bloqueados.
                                                            </p>
                                                        </div>
                                                    </div>
                                                )}
