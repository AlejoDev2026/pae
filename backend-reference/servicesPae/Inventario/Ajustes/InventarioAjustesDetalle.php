...
try {
    $idDocumento = intval(aj_parametro("idDocumento", aj_parametro("idAjuste", aj_parametro("id", 0))));

    if ($idDocumento <= 0) {
        aj_responder("no", "Debe enviar el ajuste que desea consultar");
    }

    $documentoFila = aj_obtener_fila("
        SELECT
            d.id,
            d.idTipoDocumento,
            d.idUsuarioRegistro,
    ...
    if (!$documentoFila) {
        aj_responder("no", "El ajuste no existe", [], [], 404);
    }

    if (!aj_es_tipo_ajuste($documentoFila)) {
        aj_responder("no", "El documento consultado no corresponde a un ajuste de inventario", [], [], 400);
    }

    $documento = aj_formatear_documento($documentoFila);

    $detalles = aj_obtener_filas("
        SELECT
    ...
    $detalleData = array_map(function ($fila) {
        return [
            ...
            "vencido" => aj_lote_vencido($fila["fechaVencimiento"]),
    ...
    aj_responder(
        "si",
        "Detalle de ajuste consultado correctamente",
        [
            "documento" => $documento,
            "ajuste" => $documento,
    ...
    aj_responder(
        "no",
        "Error consultando detalle de ajuste",
        [],
        ["error" => $e->getMessage()],
        500
    );
