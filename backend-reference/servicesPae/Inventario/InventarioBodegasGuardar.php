...
if ($id > 0) {
  ...
  if ($existe) {
    jexit(409, [
      "rpta" => false,
      "mensaje" => "Ya existe otra bodega con este código."
    ]);
  }
  ...
  if (!$stmt) {
    jexit(500, [
      "rpta" => false,
      "mensaje" => "Error preparando actualización de bodega.",
      "error" => $mysqli->error
    ]);
  }

  $stmt->bind_param("sssii", $codigo, $nombre, $descripcion, $estado, $id);

  if (!$stmt->execute()) {
    jexit(500, [
      "rpta" => false,
      "mensaje" => "Error actualizando bodega.",
      "error" => $stmt->error
    ]);
  }
  ...
  if ($existe) {
    jexit(409, [
      "rpta" => false,
      "mensaje" => "Ya existe una bodega con este código."
    ]);
  }

  $sql = "
    INSERT INTO BodegasInventario
  ...
  if (!$stmt) {
    jexit(500, [
      "rpta" => false,
      "mensaje" => "Error preparando creación de bodega.",
      "error" => $mysqli->error
    ]);
  }

  $stmt->bind_param("sssi", $codigo, $nombre, $descripcion, $estado);

  if (!$stmt->execute()) {
    jexit(500, [
      "rpta" => false,
      "mensaje" => "Error creando bodega.",
      "error" => $stmt->error
    ]);
  }
...
if (!$stmtGet) {
  jexit(500, [
    "rpta" => false,
    "mensaje" => "Bodega guardada, pero no se pudo consultar el registro.",
    "error" => $mysqli->error
  ]);
}
...
if (!$stmtGet->execute()) {
  jexit(500, [
    "rpta" => false,
    "mensaje" => "Bodega guardada, pero ocurrió un error consultando el registro.",
    "error" => $stmtGet->error
  ]);
}
