...
if ($id <= 0) {
  jexit(400, [
    "rpta" => false,
    "mensaje" => "El id del código de barras es obligatorio."
  ]);
}
...
if (!$stmtExiste) {
  jexit(500, [
    "rpta" => false,
    "mensaje" => "Error preparando validación del código de barras.",
    "error" => $mysqli->error
  ]);
}
...
if (!$stmtExiste->execute()) {
  jexit(500, [
    "rpta" => false,
    "mensaje" => "Error ejecutando validación del código de barras.",
...
$principal = (int)$existe["principal"];

if ($estado === 0 && $principal === 1) {
  $sql = "
...
if (!$stmt->execute()) {
  jexit(500, [
    "rpta" => false,
    "mensaje" => "Error cambiando estado del código de barras.",
    "error" => $stmt->error
  ]);
}
...
jexit(200, [
  "rpta" => true,
  "mensaje" => $estado === 1 ? "Código de barras activado correctamente." : "Código de barras inactivado correctamente.",
  "data" => [
    "id" => $id,
    "estado" => $estado
  ]
