...
/*
  Este servicio es exclusivo para Roldanillo.
*/
...
if (!in_array($tipoPeriodo, ["DIARIO", "SEMANAL", "MENSUAL"], true)) {
  jexit(400, [
    "rpta" => "no",
...
try {
  ...
  if (!$st) {
    throw new Exception("Error prepare INSERT despacho Roldanillo: " . $mysqli->error);
  }
  ...
  if (!$st->execute()) {
    throw new Exception("Error execute INSERT despacho Roldanillo: " . $st->error);
  }
  ...
  jexit(200, [
    "rpta" => "si",
    "mensaje" => "Despacho Roldanillo creado correctamente",
    "data" => [
  ...
  jexit(500, [
    "rpta" => "no",
    "mensaje" => "No se pudo crear el despacho Roldanillo",
    "error" => $e->getMessage()
  ]);
}